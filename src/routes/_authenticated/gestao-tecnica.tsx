// GESTÃO TÉCNICA — a mesa do gestor da equipe de campo (R299, R300, R304).
//
// Davi, 15/09/2026: "Vamos alterar a tela 'Sobreaviso' para 'Gestão Técnica'";
// "Note que o Vinicius passará a utilizar bastante esta tela pois nela
// estarão as principais informações sobre o trabalho da equipe de campo."
//
// A ORDEM DA PÁGINA É A ORDEM DO DIA DELE (R300): primeiro os INDICADORES
// (o dashboard que morava na Operacional, recolhível), depois a FILA DE
// DECISÃO — o que espera uma decisão do gestor hoje: retorno a marcar e
// cobrança a lançar —, e só então o PLANTÃO, que ele monta duas vezes por
// mês, também recolhível. Equipes e Fechamentos moram na barra da tela.
//
// A CHAVE DE PERMISSÃO continua `sobreaviso` (telas.ts): é o que está gravado
// na matriz. /sobreaviso redireciona para cá preservando ?mes=&dia=&visao=.
//
// O que segue é a tela do Sobreaviso (R116, U86), que virou a seção do
// plantão desta página — os comentários dela continuam valendo.
//
...
import {
  CalendarDays, CalendarRange, ChevronLeft, ChevronRight, ChevronsDownUp, ChevronsUpDown,
  Eraser, FileDown, LayoutGrid, Receipt, Users,
} from "lucide-react";
...
import { FONT, card, goldButton, rotuloDeSecao, botaoDaBarra, pilulaDaBarra } from "@/lib/ui";
...
// R299: o dashboard que morava na Operacional Técnica, e as portas que vieram
// com ele (Equipes) e do Administrativo (Fechamentos).
import { DashboardOperacional } from "@/features/paineis/DashboardOperacional";
// R300: o que espera uma decisão do gestor HOJE.
import { FilaDeDecisao } from "@/features/paineis/FilaDeDecisao";
import { DialogoEquipes } from "@/features/duplas/DialogoEquipes";
import { PainelChamado } from "@/features/chamados/PainelChamado";

export const Route = createFileRoute("/_authenticated/gestao-tecnica")({
  beforeLoad: async () => {
  ...
  component: GestaoTecnicaPage,
});

/**
 * R299/R300: as duas faixas RECOLHEM, e ficam recolhidas — o mecanismo da
 * Início (R175) e da Operacional (R296). Chaves PRÓPRIAS desta tela: recolher
 * os indicadores aqui não pode recolher os de lá, e recolher o plantão não
 * recolhe os indicadores. O plantão nasce ABERTO também: a primeira visita
 * mostra a página inteira, e quem monta a escala duas vezes por mês decide.
 */
const CHAVE_PAINEL_GT = "prever-gestao-painel";
const CHAVE_PLANTAO_GT = "prever-gestao-plantao";

function GestaoTecnicaPage() {
  const { theme } = useTheme();
  const isLight = theme === "light";
  const busca = Route.useSearch();
  const navegar = Route.useNavigate();
  // para as OUTRAS rotas (Fechamentos, Operacional recortada, a página do chamado)
...
// ── O PERÍODO: semana ou mês (R253) ───────────────────────────────────────
  ...
  // R299: a faixa de indicadores recolhe (preferência gravada, chave própria).
  const [painelAberto, setPainelAberto] = useState(() => {
    try { return localStorage.getItem(CHAVE_PAINEL_GT) !== "fechado"; } catch { return true; }
  });
  useEffect(() => {
    try { localStorage.setItem(CHAVE_PAINEL_GT, painelAberto ? "aberto" : "fechado"); } catch { /* modo privado */ }
  }, [painelAberto]);
  // R300: o plantão também recolhe — é o que se monta duas vezes por mês.
  const [plantaoAberto, setPlantaoAberto] = useState(() => {
    try { return localStorage.getItem(CHAVE_PLANTAO_GT) !== "fechado"; } catch { return true; }
  });
  useEffect(() => {
    try { localStorage.setItem(CHAVE_PLANTAO_GT, plantaoAberto ? "aberto" : "fechado"); } catch { /* modo privado */ }
  }, [plantaoAberto]);
  // R299 B.3: a janela de EQUIPES, que veio da Operacional com o dashboard.
  const [duplasAberto, setDuplasAberto] = useState(false);
  // o chamado aberto pela fila de decisão ou pelas obras do dashboard (R33:
  // desliza no painel lateral; quem decidiu dez continua olhando a fila)
  ...
  return (
    ...
    <div className="sangra-x" style={{ paddingTop: 4, paddingBottom: 40, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <span style={{ flex: 1 }} />
        {/* R299 B.2 — FECHAMENTOS veio do Administrativo: "Na página
            Administrativo, mova o botão 'Fechamentos' para a página 'Gestão
            Técnica'". Some para quem a matriz não deixa entrar (botão para
            porta trancada é armadilha). Pílula na régua do DS §6.26. */}
        {podeVer("fechamentos") !== false ? (
          <button
            type="button"
            onClick={() => navigate({ to: "/fechamentos" })}
            title="Fechamentos — as cobranças do mês, por competência"
            style={pilulaDaBarra(isLight, textPrimary)}
          >
            <Receipt size={15} /> Fechamentos
          </button>
        ) : null}
        {/* R299 B.3 — a porta das EQUIPES DE CAMPO, que veio da Operacional
            junto com o dashboard que mostra equipes. */}
        <button
          type="button"
        ...
        {/* R299 B.1 — o recolher dos indicadores: "Mova também o botão de
            expandir e recolher o dashboard". */}
        <button
          type="button"
      ...
      {/* ── OS INDICADORES (R299) — o dashboard da equipe de campo ─────────
          Três colunas, duas faixas (DASHBOARD.md). O clique num KPI abre a
          Operacional Técnica JÁ RECORTADA por ele (`?kpi=`): quem conta é
          quem filtra, mesmo com a lista em outra tela. Só desktop — no
          celular esta página é a projeção do plantão do dia (link com ?dia=). */}
      {painelAberto ? (
        <div className="so-desktop" style={{ flexDirection: "column", gap: 14 }}>
          <DashboardOperacional
            aoEscolherKpi={(kpi) => navigate({ to: "/painel/operacional", search: { kpi } as any })}
            aoAbrirChamado={(id) => setPainelId(id)}
          />
        </div>
      ) : null}

      {/* ── A FILA DE DECISÃO (R300) — o que o gestor olha TODO DIA ────────
          "os cards das atividades aguardando retorno, ou aguardando o
          Vinicius lançar cobrança (Ou não). Este campo é importante e o
          Vinicius olhará todos os dias." Vem ANTES do plantão de propósito. */}
      <FilaDeDecisao
        aoAbrir={(id) => setPainelId(id)}
        aoVerCobrancas={() => navigate({ to: "/painel/operacional", search: { kpi: "aguardando_conferencia" } as any })}
      />

      {/* ── O PLANTÃO (R116/R253/R254) — a seção que se monta 2x por mês ───
          R300: "o campo de montagem do calendário de plantão, que o Vinicius
          vai montar 2x ao mês provavelmente (Ou seja, o uso será menor)".
          Rótulo de seção, navegação do período, as DUAS visões, o PDF e o
          recolher; o conteúdo abaixo só existe com a seção aberta. */}
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={rotuloDeSecao(isLight)}>Plantão</span>
        ...
        <button
          type="button"
          onClick={() => setPlantaoAberto((v) => !v)}
          aria-pressed={!plantaoAberto}
          title={plantaoAberto ? "Recolher o plantão" : "Mostrar o plantão"}
          aria-label={plantaoAberto ? "Recolher a seção do plantão" : "Mostrar a seção do plantão"}
          style={{ ...botaoDaBarra(isLight, textPrimary), width: 34, height: 34, borderRadius: 10 }}
        >
          {plantaoAberto ? <ChevronsDownUp size={15} /> : <ChevronsUpDown size={15} />}
        </button>
      </div>
      ...
      {/* R299: as equipes de campo (a janela que veio da Operacional) e o
          painel lateral do chamado que a fila e as obras abrem (R33). */}
