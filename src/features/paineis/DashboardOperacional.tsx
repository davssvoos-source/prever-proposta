// O DASHBOARD DA EQUIPE TÉCNICA DE CAMPO — R125, R299.
//
// Nasceu na Operacional Técnica (R27/R68/R125) e SAIU dela na R299 (Davi,
// 15/09/2026: "O Dashboard e KPIs da página Operacional Técnica trocarão de
// lugar, agora deverão estar em 'Gestão Técnica'"). É componente, e não
// tela, porque a hospedeira decide três coisas que não são do dashboard:
// se ele está recolhido, o que o clique num KPI faz, e onde o chamado da
// obra abre.
//
// A ESTRUTURA VEM DE docs/DASHBOARD.md:
//   · FAIXAS de painéis com ALTURA ÚNICA (§4) — a constante que faz a
//     fileira ler como uma peça só. Duas faixas de 168.
...
//     MESMA função pura — `chamadosDoKpi`, em indicadores.ts. Com a lista em
//     OUTRA tela (R299), o clique entrega a CHAVE à hospedeira, e a Operacional
//     Técnica a lê da URL (`?kpi=`) e recorta com a mesma função.
//
...
import { useTecnicos, useVeFinanceiro } from "@/features/gerencial/data";
...
import {
  ...
  abertosPorCliente, abertosPorTipo,
...
interface Props {
  /** O clique num KPI (inclusive "aguardando_conferencia"). A hospedeira decide o que abrir. */
  aoEscolherKpi: (kpi: ChaveKpiOperacional) => void;
  /** O clique numa obra: o chamado abre onde a hospedeira mandar (o painel lateral dela). */
  aoAbrirChamado: (id: string) => void;
  /** Qual KPI está recortando uma lista em algum lugar — desenha o anel (§7.1). */
  kpiAtivo?: ChaveKpiOperacional | null;
}

export function DashboardOperacional({ aoEscolherKpi, aoAbrirChamado, kpiAtivo = null }: Props) {
   ...
   * Todo chamado de campo nasce com `equipe: "tecnica"` (chamados/data.ts), e
   * nada no banco impede um de outra equipe (o T.I. em campo) — o recorte é
   * explícito, e é o que faz este ser o painel do Vinicius.
   */
  ...
  // nomes dos clientes para o gráfico por cliente — só id/nome. A MESMA chave
  // da Operacional: o react-query devolve a mesma resposta às duas telas.
  ...
  // um momento só para todas as contas do render — KPIs e indicadores
  // precisam concordar sobre "agora", ou um chamado no limite do prazo
  // poderia contar diferente em duas peças da mesma faixa
        ...
        <div style={{ display: "flex", gap: GAP, alignItems: "stretch", flexWrap: "wrap" }}>
          ...
          <div style={{
            ...
            {kpis.map((k) => {
              ...
              return (
                <button
                  key={k.chave}
                  // R299: quem conta é quem filtra — mas a lista não está mais
                  // nesta tela. O clique entrega a chave à hospedeira, que na
                  // Gestão Técnica navega para a Operacional já recortada.
                  onClick={() => aoEscolherKpi(k.chave)}
                  aria-pressed={selecionado}
                  title={`${k.valor} — clique para abrir a lista na Operacional Técnica`}
                  className="elevavel kpi-tile ruido"
                  style={{
                    ...base, borderRadius: 14, padding: "6px 8px",
                    display: "flex", flexDirection: "column",
          ...
          <div style={{
            ...
            <Tile
              ...
              title={`${aguardandoConferencia} — clique para abrir a lista na Operacional Técnica`}
              aoClicar={() => aoEscolherKpi("aguardando_conferencia")}
            />
          </div>
        </div>
