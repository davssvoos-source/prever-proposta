...
export function DetalheInterno({ id }: { id: string }) {
  ...
  // ── R234 (U120): O POSTO DE TRABALHO ─────────────────────────────────────
  // Davi, 08/09/2026: "a tela de configuração da atividade deve ter os campos
  // de 'Descrição' ou 'Problema' e 'Solução' como os campos que ocupam a maior
  // parte do espaço da tela, pois são os principais campos que o usuário vai
  // interagir. […] eu vejo que nós temos uma tela criada inicialmente para
  // celular, que os botões simplesmente foram esticados para adaptar para PC. E
  // não é isso que eu quero, quero algo essencialmente para desktop."
  //
  // O que estava errado, medido na tela dele (1920px): a grade era 3fr|2fr, e a
  // coluna "estreita" tinha 660px de SELETORES esticados — mais larga que o
  // texto de muitas telas — enquanto a Descrição, que é onde a pessoa trabalha,
  // ficava com 985px e rolava. E a página, com `.pagina-larga`, encostava nas
  // duas bordas da janela (a R228 deu largura e tirou a margem).
  //
  // O desenho novo, em quatro faixas de cima para baixo:
  //   1. CABEÇALHO — quem é esta atividade + o progresso (a rosca, R235)
  //   2. PROPRIEDADES — uma FAIXA horizontal de campos compactos. Status, tipo,
  //      impacto, quando, responsável, apoio e equipes deixam de ser uma coluna
  //      alta de caixas de 660px e passam a ser sete controles do tamanho do
  //      próprio conteúdo, lidos numa varredura só.
  //   3. CORPO — os TEXTOS ocupam a faixa inteira (na corretiva, Problema e
  //      Solução lado a lado a partir de 1440px), com uma coluna de contexto
  //      (cliente, arquivos) de 320-400px à direita.
  //   4. EQUIPAMENTOS (R236) e a CONVERSA — embaixo, onde se desce para ler.
  ...
  const ROTULO_BARRA: CSSProperties = { ...LABEL, marginBottom: 0 };
  const DICA: CSSProperties = {
    fontFamily: "var(--fonte)", fontWeight: 400, fontSize: 11, color: textSecondary,
    lineHeight: 1.4, marginTop: -6,
  };
  /**
   * Um item da faixa de propriedades: rótulo curto em cima, controle embaixo,
   * largura-base própria (0 = do tamanho do conteúdo). É FUNÇÃO, não
   * componente: um componente declarado aqui dentro nasceria com identidade
   * nova a cada render e o React remontaria o campo — tirando o foco de quem
   * está digitando.
   */
  const prop = (rotulo: string, base: number, filho: ReactNode) => (
    <div style={{
      display: "flex", flexDirection: "column", gap: 5, minWidth: 0,
      flex: base > 0 ? `0 1 ${base}px` : "0 0 auto",
    }}>
      <span style={ROTULO_BARRA}>{rotulo}</span>
      {filho}
    </div>
  );
  ...
  return (
    <div className="pagina-trabalho" style={{ paddingTop: 12, paddingBottom: 56, color: textPrimary }}>
      <div className="trabalho-miolo">

        {/* ══ 1. CABEÇALHO — a identidade e o progresso ═════════════════════ */}
        <header style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            {/* R195: título de página de 22px é 700 — o verificador cobra */}
            <h1 style={{
              margin: 0, fontFamily: "var(--fonte)", fontWeight: 700, fontSize: 22,
              lineHeight: 1.25, textWrap: "pretty" as any,
            }}>
              {chamado.titulo}
            </h1>
            <div style={{
          ...
          {/* R235: "no canto superior direito da tela, adicione um campo
              contendo um gráfico de rosca" — o progresso é a única coisa que
              responde "e aí, andou?" sem ler a atividade inteira. */}
          <div style={{ ...card(isLight), padding: "10px 16px 10px 10px", flexShrink: 0 }}>
            <RoscaDeProgresso p={prog} tamanho={92} />
          </div>
        </header>

        {/* ══ 2. PROPRIEDADES — a faixa, não uma coluna de caixas ═══════════ */}
        <div style={{ ...CARD, padding: "14px 16px", gap: 12 }}>
          <div className="atividade-props">
            {prop("Status", 190, (
              <SeletorDeOpcao
                id="det-status"
            ...
            {temImpacto(chamado.tipo) && prop("Impacto operacional", 210, (
              <SeletorDeOpcao
                id="det-impacto"
            ...
            {prop("Quando", 230, (
              <>
                <CampoQuando
                  ...
                  nota={reagendado}
                  estiloEntrada={{ ...INPUT, height: 36, fontSize: 12.5 }}
            ...
            {equipesEnvolvidas.length > 0 && prop("Equipes envolvidas", 0, (
            ...
            {chamado.tipo === "implantacao" && prop("Proposta comercial aprovada", 260, (
              <CampoComBusca
                id="det-proposta"
                compacto
          ...
          {/* R144: o RECEBIMENTO — quem criou e quando; início e conclusão são
              os carimbos do banco. Era um bloco na coluna; virou o rodapé fino
              da faixa, que é o lugar de informação que não se edita. */}
          <div style={{
            display: "flex", flexWrap: "wrap", gap: "2px 18px", paddingTop: 10,
        ...
        {/* ══ 3. O CORPO — o texto manda ════════════════════════════════════ */}
        <div className="atividade-corpo">
          <div className={ehCorretiva ? "atividade-textos duplo" : "atividade-textos"}>
            {/* Descrição — o editor de blocos (R135): caixa de marcar de
                verdade, ponto de lista, negrito/itálico, menção com "@". Grava
                sozinho. Na corretiva ela é a "descrição do problema
                detectado" (R149). */}
            ...
            {/* R149: a corretiva tem DOIS textos — o problema e a solução
                aplicada. A solução mora em `servico_executado`, a mesma coluna
                que o chamado de campo já usa para "o que foi feito para
                resolver". Lado a lado no desktop grande: na corretiva a pessoa
                escreve na Solução LENDO o problema. */}
            {ehCorretiva && (
              <div style={{ ...CARD, minWidth: 0 }}>
          ...
          {/* a coluna de CONTEXTO: de quem é o trabalho e o que veio com ele */}
          <aside style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}>
            {/* R143/R151: cliente(s), GRUPO(s) de clientes ou interno — mais de
                um cliente na mesma atividade, aqui como no painel lateral */}
            <div style={CARD}>
        ...
        {/* ══ 4a. EQUIPAMENTOS — a faixa larga, porque o gesto é arrastar ═══ */}
        {/* R226 (U119) + R236 (U120): cliente ÚNICO → o patrimônio do cliente
            de um lado, o que está fora dele do outro, e o vínculo se faz
            arrastando — o mesmo esquema da ficha (R206), que o Davi citou como
            o jeito certo. Precisa de largura: fica numa faixa própria. */}
        ...
        {/* ══ 4b. A CONVERSA e o HISTÓRICO ══════════════════════════════════ */}
        <div className="atividade-conversa">
