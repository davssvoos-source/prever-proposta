// NOVA VISITA TÉCNICA — o formulário da proposta comercial (R194, R214).
//
// Até a U116 este arquivo era a ROTA /gerencial/nova inteira. Na U117 o
// formulário virou componente, porque o Davi pediu que a Proposta Comercial
// EXPANDA dentro do "+" da Início como os outros tipos de demanda (R214):
// "ao criar uma atividade do tipo de demanda Proposta Comercial, os campos para
// criação da proposta devem expandir no campo de criação igual aos outros tipos
// de demanda." A rota continua existindo (gerencial.nova.tsx) e é só a casca.
//
// O que NÃO mudou — e o verificador cobra letra a letra: as guardas do endereço
// da U84 (editar zera a conferência e a coordenada; campo, botão e seletor de
// cliente travam durante a busca; "O mapa entendeu" impresso), as três colunas
// da R194, o "o que falta para agendar" dito na tela, o cliente equivalente e
// a fachada herdada. Embutido, o cabeçalho de página some (o diálogo tem o
...
export interface NovaVisitaTecnicaProps {
  /** o técnico já escolhido — a segunda pergunta do "+" da Início (R214) */
  tecnicoInicial?: string | null;
  /** chamado depois de a visita ser agendada */
  aoConcluir?: () => void;
  /** o botão de voltar do cabeçalho — só fora do embutido */
  aoVoltar?: () => void;
  /** dentro do diálogo do "+": sem sangria, sem cabeçalho de página, no máximo duas colunas */
  embutido?: boolean;
}

export function NovaVisitaTecnica({ tecnicoInicial = null, aoConcluir, aoVoltar, embutido = false }: NovaVisitaTecnicaProps) {
